import { NextRequest, NextResponse } from 'next/server';
import { requireAdminRole } from '@/lib/admin/admin-auth';
import { TemplatesService } from '@/lib/admin/services/templates-service';
import { AdminErrorCode, createAdminError } from '@/lib/admin/admin-errors';

const templatesService = new TemplatesService();

/**
 * POST /api/admin/templates/import
 * Import templates from uploaded file
 */
export async function POST(request: NextRequest) {
    try {
        // Require admin authentication
        const user = await requireAdminRole(request);

        const formData = await request.formData();
        const file = formData.get('file') as File;

        if (!file) {
            throw createAdminError(AdminErrorCode.INVALID_INPUT, 'Không tìm thấy file upload');
        }

        // Validate file type
        const allowedTypes = ['application/json', 'text/csv'];
        if (!allowedTypes.includes(file.type)) {
            throw createAdminError(AdminErrorCode.FILE_UPLOAD_ERROR, 'Chỉ hỗ trợ file JSON và CSV');
        }

        // Validate file size (10MB max)
        if (file.size > 10 * 1024 * 1024) {
            throw createAdminError(AdminErrorCode.FILE_UPLOAD_ERROR, 'File không được lớn hơn 10MB');
        }

        // Read file content
        const fileContent = await file.text();
        let templatesData: any[];

        try {
            if (file.type === 'application/json') {
                const jsonData = JSON.parse(fileContent);
                // Handle both direct array and wrapped format
                templatesData = Array.isArray(jsonData) ? jsonData : jsonData.templates || jsonData.data || [];
            } else if (file.type === 'text/csv') {
                // Simple CSV parsing (in production, use a proper CSV parser)
                const lines = fileContent.split('\n');
                const headers = lines[0].split(',').map(h => h.trim());
                templatesData = lines.slice(1)
                    .filter(line => line.trim())
                    .map(line => {
                        const values = line.split(',').map(v => v.trim());
                        const obj: any = {};
                        headers.forEach((header, index) => {
                            obj[header] = values[index] || '';
                        });
                        return obj;
                    });
            } else {
                throw new Error('Định dạng file không được hỗ trợ');
            }
        } catch (parseError) {
            throw createAdminError(AdminErrorCode.FILE_UPLOAD_ERROR, 'Không thể đọc nội dung file');
        }

        if (!Array.isArray(templatesData) || templatesData.length === 0) {
            throw createAdminError(AdminErrorCode.INVALID_INPUT, 'File không chứa dữ liệu template hợp lệ');
        }

        // Import templates
        const result = await importTemplates(templatesData, user.id);

        return NextResponse.json(result);
    } catch (error) {
        console.error('Error in POST /api/admin/templates/import:', error);

        if (error instanceof Error && 'statusCode' in error) {
            return NextResponse.json(
                { error: error.message, code: (error as any).code },
                { status: (error as any).statusCode }
            );
        }

        return NextResponse.json(
            { error: 'Lỗi server nội bộ', code: AdminErrorCode.INTERNAL_SERVER_ERROR },
            { status: 500 }
        );
    }
}
/**

 * Import templates from data array
 */
async function importTemplates(templatesData: any[], userId: string): Promise<{
    success: number;
    failed: number;
    errors: Array<{ row: number; error: string; data?: any }>;
}> {
    const result = {
        success: 0,
        failed: 0,
        errors: [] as Array<{ row: number; error: string; data?: any }>
    };

    for (let i = 0; i < templatesData.length; i++) {
        try {
            const templateData = templatesData[i];

            // Create the template
            await templatesService.createTemplate(templateData, userId);
            result.success++;
        } catch (error) {
            result.failed++;
            result.errors.push({
                row: i + 1,
                error: error instanceof Error ? error.message : 'Lỗi không xác định',
                data: templatesData[i]
            });
        }
    }

    return result;
}