...
async function main() {
    ...
    const prompt1 = await prisma.generatedPrompt.create({
        data: {
            ...
            generatedText:
                'Bạn là một giáo viên Toán học chuyên nghiệp. Hãy tạo một kế hoạch bài dạy 5 cột cho chủ đề "Phương trình bậc nhất một ẩn" dành cho học sinh lớp 7 theo chuẩn GDPT 2018...',
            targetTool: "chatgpt",
    ...
    const prompt2 = await prisma.generatedPrompt.create({
        data: {
            ...
            generatedText:
                'Bạn là một giáo viên Ngữ văn có kinh nghiệm. Hãy tạo kế hoạch bài dạy 4 cột cho chủ đề "Tả người" dành cho học sinh lớp 6...',
            targetTool: "gemini",
    ...
    const sharedContent1 = await prisma.sharedContent.create({
        data: {
            ...
            description:
                "Kế hoạch bài dạy chi tiết cho chủ đề phương trình bậc nhất một ẩn, áp dụng phương pháp dạy học tích cực",
    ...
    const sharedContent2 = await prisma.sharedContent.create({
        data: {
            ...
            description:
                "Dàn ý slide thuyết trình về kỹ năng tả người trong văn học, phù hợp với học sinh lớp 6",
    ...
    console.log(
        `Created users: ${teacher1.name}, ${teacher2.name}, ${teacher3.name}`
    );
